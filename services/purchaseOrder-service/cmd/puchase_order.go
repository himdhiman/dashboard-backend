...
func InitializePurchaseOrderService(router *gin.Engine, ctx context.Context, config *config.PurchaseOrderServiceConfig, logger logger.ILogger, mongoClient mongo.IMongoClient, taskManager *task.TaskManager) (*PurchaseOrderServices, error) {
	...
	purchaseOrderService := services.NewPurchaseOrderService(logger, mapper, taskManager, &purchaseOrderRepo, &purchaseOrderProductsRepo, *config.ProductService, *config.ShippingService)
