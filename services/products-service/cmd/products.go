...
import (
	...
	conflux "github.com/himdhiman/dashboard-backend/libs/conflux"
	"github.com/himdhiman/dashboard-backend/libs/mongo/repository"
)

func InitializeProductsService(router *gin.Engine, config *config.ProductsServiceConfig, logger logger.ILogger, cache cache.Cacher, confluxService *conflux.ConfluxService, mongoClient mongo.IMongoClient) *gin.Engine {
	...
	router = routes.AddProductsRoutes(router, logger, unicommerceProductsService, productsService)
