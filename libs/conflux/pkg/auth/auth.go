...
func NewAuthentication(cache cache.Cacher, logger logger.ILogger, crypto *crypto.Crypto, authConfig AuthConfig) *Authentication {
...
func (a *Authentication) FetchTokens(ctx context.Context, apiCode string) (*models.TokenResponse, error) {
	...
	if err := a.cacheGet(ctx, a.AuthConfig.BaseURL, &endpoint); err != nil {
		return nil, fmt.Errorf("failed to fetch endpoint for api %s: %w", apiCode, err)
	}

	if err := a.cacheGet(ctx, a.AuthConfig.AuthPath, &path); err != nil {
		return nil, fmt.Errorf("failed to fetch path for api %s: %w", apiCode, err)
	}

	if err := a.cacheGet(ctx, a.AuthConfig.Credentials, &creds); err != nil {
